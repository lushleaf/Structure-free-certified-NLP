...
class AmazonProcessor(DataProcessor):
    ...
    def _generate_train_example(self, data_dir, sub_dir):
        ...
        for _ in tqdm(range(n_train)):
            # re-organize the label range
            # print(data.loc[_, :]);input()
            instance_label = data.loc[_, 0] - 1
            # load sentence
            text_a = data.loc[_, 2]
            # generate data id
            guid = "%s-%d" % (sub_dir, _)
            # no encoder-decoder, only text_a
            text_b = None
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=instance_label))
        return examples
